 ...
 */
#define PLAYER_NAME Aitor_Menta
    ...
    while (not cua_pos.empty() and not trobat) {        // mentre no hi hagi posicions o  no hagi trobat res busco
        ...
        if (cell(p).bonus == Money or (c.type == Warrior and cell(p).weapon != NoWeapon) or (cell(p).bonus == Food and c.life < 30)) {
            ...
            for (Dir d : dirs) {
                if (pos_ok(p+d)) {
                    Pos p2(p+d);
                    if (not visitat[p2.i][p2.j] and cell(p2).type == Street and cell(p2).id == -1 and (cell(p2).b_owner == me() or cell(p2).b_owner == -1)) {        // si no he visitat la posicio i no tenim cap element que ens bloquegi ens guardem aquest lloc
